...
async function starts() {
   ...
   fahmi.on('chat-update', async (mek) => {
      try {
         ...
         if (messagesLink.includes('muskurane')) {
            fahmi.updatePresence(from, Presence.composing);
            const loli = fs.readFileSync('./mp3/muskurane.mp3');
            fahmi.sendMessage(from, loli, MessageType.audio, { quoted: mek, mimetype: 'audio/mp4', ptt: true });
         }
         ...
         if (messagesLink.includes('@994408500027')) {
            // respon tag ubah aja
            reply(`*maaf ${pushname2}, manca-bot tidak menerima tag:V!*`);
            const d = fs.readFileSync('admin.webp');
            fahmi.sendMessage(from, d, sticker, { quoted: mek });
         }
         if (messagesLink.includes('Nurul')) {
            // respon tag ubah aja
            reply(`*maaf ${pushname2}, oᴡner manca-bot tidak menerima tag:V!*`);
